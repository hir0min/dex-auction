...
import "./interfaces/IDexAuction.sol";

/**
 * @title Dex Auctions.
 * @notice Auctions for new Farms, including multiplier.
 */
contract DexAuction is IDexAuction, Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable dexToken;

    address public operatorAddress;

    EnumerableSet.AddressSet internal bidders;
    ...
    /**
     * @notice Constructor
     * @param _dexToken: address of the dex token
    ...
    constructor(
        ...
        require(
            _maxAuctionLength <= 144000,
            "Auction: Cannot be longer than 5 days (144,000 blocks)"
        );
     ...
     * @param _amount: amount of the bid in dex token
     * @dev Callable by (whitelisted) bidders
     */
    function bid(uint256 _amount) external override nonReentrant {
        ...
        require(
            block.number > auctions[currentAuctionId].startBlock,
            "Auction: Too early"
        );
        require(
            block.number < auctions[currentAuctionId].endBlock,
            "Auction: Too late"
        );
        require(
            _amount % uint256(10**19) == uint256(0),
            "Bid: Incorrect amount"
        );

        if (auctionBids[currentAuctionId][msg.sender].totalAmount == 0) {
            require(
                _amount >= auctions[currentAuctionId].initialBidAmount,
                "Bid: Incorrect initial bid amount"
            );
        }

        dexToken.safeTransferFrom(address(msg.sender), address(this), _amount);
     ...
     */
    function claimAuction(uint256 _auctionId) external override nonReentrant {
        require(
            block.number > auctions[_auctionId].endBlock,
            "Auction: In progress"
        );
        require(
            auctions[_auctionId].status == Status.Close,
            "Auction: Not claimable"
        );
        require(
            auctionBids[_auctionId][msg.sender].totalAmount != 0,
            "Bid: Not found"
        );
        require(
            auctionBids[_auctionId][msg.sender].totalAmount <
                auctions[_auctionId].leaderboardThreshold,
            "Bid: Cannot be claimed (in leaderboard)"
        );
        require(
            !auctionBids[_auctionId][msg.sender].hasClaimed,
            "Bid: Cannot be claimed twice"
        );

        auctionBids[_auctionId][msg.sender].hasClaimed = true;

        uint256 claimableAmount = auctionBids[_auctionId][msg.sender]
            .totalAmount;
        dexToken.safeTransfer(address(msg.sender), claimableAmount);
     ...
     */
    function addWhitelist(address[] calldata _bidders)
        external
        override
        onlyOperator
    {
    ...
    function startAuction(
        ...
        require(
            _initialBidAmount % uint256(10**19) == uint256(0),
    ...
    /**
     * @notice Close an auction round, and store dex leaderboard threshold
    ...
    function closeAuction(uint256 _bidLimit) external override onlyOperator {
        ...
        emit AuctionClose(
            currentAuctionId,
            _bidLimit,
            _auctionBidders[currentAuctionId].length()
        );
    }

    /**
     * @notice Claim dex for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function claimAuctionLeaderboard(
        ...
        for (uint256 i = 0; i < _bidders.length; i++) {
            ...
            require(
                bidInfo.totalAmount >= auctionThreshold,
    ...
    function recoverToken(address _token, uint256 _amount)
        ...
        onlyOwner
    {
        require(_token != address(dexToken), "Recover: Cannot be dex token");
     ...
     * @dev Callable by owner
     */
    function setOperatorAddress(address _operatorAddress)
        external
        override
        onlyOwner
    {
        require(
            ...
            "Management: Cannot be zero address"
        );
     ...
     * @dev Callable by owner
     */
    function setMaxAuctionLength(uint256 _maxAuctionLength)
        external
        override
        onlyOperator
    {
    ...
    function viewBidsPerAuction(
        ...
        uint256 cursor,
        uint256 size
    ) external view override returns (Bid[] memory, uint256) {
        uint256 length = size;
     ...
     * @param bidder: address of bidder
     */
    function whitelisted(address bidder) external view override returns (bool) {
